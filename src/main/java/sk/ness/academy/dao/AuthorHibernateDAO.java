...
public class AuthorHibernateDAO implements AuthorDAO {
  ...
  public List<AuthorStats> findAllWithNumArt() {
    return this.sessionFactory.getCurrentSession().createSQLQuery("select distinct a.author as authorName, count(a.id) as articleCount from articles a group by a.author")
